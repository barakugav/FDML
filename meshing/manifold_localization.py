...
def get_ray_distance_to_wall(polygon, x, y, theta):
    ...
    for i in range(len(rotated_polygon)):
        ...
        q = [rotated_polygon[(i+1) % len(rotated_polygon)][0], rotated_polygon[(i+1) % len(rotated_polygon)][1]]
...
def project_polygon_soup_to_xy(filename, triangles, vertices, N):
    ...
    for tri in triangles:
        ...
        new_tri = np.array([
            [int(a[0] / (N-1) * IMG_SIZE), IMG_SIZE - int(a[1] / (N-1) * IMG_SIZE)],
            [int(b[0] / (N-1) * IMG_SIZE), IMG_SIZE - int(b[1] / (N-1) * IMG_SIZE)],
            [int(c[0] / (N-1) * IMG_SIZE), IMG_SIZE - int(c[1] / (N-1) * IMG_SIZE)],
        ])
...
if __name__ == "__main__":
    ...
    f_d2_ = lambda x, y, theta: f_d2(x, y, theta + beta)
    # f_isect = intersect_manifolds_with_shift(f_d1, f_d2, r, beta)
    f_isect = intersect_manifolds_with_rotate(f_d1, f_d2, beta)
    
    vertices, triangles = implicit_manifold_to_polygon_soup(f_d1, crit, N, minn, maxx)
