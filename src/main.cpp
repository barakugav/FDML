...
static void update_trapezoid_limiting_vertices_data(const Trapezoid &trapezoid,
...
static void
init_trapezoids_with_regular_vertical_decomposition(const Arrangement &arr,
	...
	std::map<Halfedge_const_handle, Vertex_const_handle, Less_edge> most_right_vertex(Less_edge(arr.geometry_traits()));
	for (const auto &v : vertices) {
		...
		if ((v_decomp_data.is_edge_above && is_free(v_decomp_data.edge_above->face())) &&
			...
			Trapezoid trapezoid(v_decomp_data.edge_above, v_decomp_data.edge_below, left_v, v);
	...
	for (const auto &v_pair : vtrapezoids) {
		...
		std::cout << "\t(" << v->point() << "):"
				  << " TopLeftT " << (int)v_data.top_left_trapezoid << " TopRightT " << (int)v_data.top_right_trapezoid
				  << " BottomLeftT " << (int)v_data.bottom_left_trapezoid << " BottomRightT "
...
static int calc_trapezoids(const std::vector<Segment> &segments) {
	...
	std::map<unsigned int, Trapezoid> trapezoids;
	std::unordered_map<Vertex_const_handle, VertexData> vtrapezoids(arr.number_of_vertices());
	for (auto v = arr.vertices_begin(); v != arr.vertices_end(); ++v)
		vtrapezoids[v] = VertexData(v->point(), arr.geometry_traits());

	// Init trapezoids with regular vertical decomposition
	init_trapezoids_with_regular_vertical_decomposition(arr, trapezoids, vtrapezoids);
	...
	for (auto &event : events) {
		...
		if (get_edge(event.v1, event.v2, v1v2_edge)) {
			...
			Trapezoid left_new(left.top_edge, left.bottom_edge, left.left_vertex, event.v1);
			...
			Trapezoid right_new(right.top_edge, right.bottom_edge, event.v2, right.right_vertex);
