if(NOT FDML_MACROS_FILE_INCLUDED)
  ...
  # Given a list in the form of "debug;<path-debug>;optimized;<path-optimized>"
  # return the directory of the path depending on the CMAKE_BUILD_TYPE value
  function(get_directory _elements _dir)
    list(LENGTH _elements _length)
    if (${_length} GREATER 1)
      # _elements is a list of length at least 2
      if(CMAKE_BUILD_TYPE MATCHES Debug)
	list (FIND _elements "debug" _index)
	if (${_index} GREATER -1)
	  # Obtain the directory from corresponding item
	  math(EXPR _index "${_index}+1")
	  list(GET _elements ${_index} _library)
	  get_filename_component(_result ${_library} DIRECTORY)
	  set (${_dir} ${_result} PARENT_SCOPE)
	else()
	  # Obtain the directory from the 2nd item
	  list(GET _elements 1 _library)
	  get_filename_component(_result ${_library} DIRECTORY)
	  set (${_dir} ${_result} PARENT_SCOPE)
	endif()
      elseif(CMAKE_BUILD_TYPE MATCHES Release)
	list (FIND _elements "optimized" _index)
	if (${_index} GREATER -1)
	  math(EXPR _index "${_index}+1")
	  list(GET _elements ${_index} _library)
	  get_filename_component(_result ${_library} DIRECTORY)
	...
	set (${_dir} ${_result} PARENT_SCOPE)
      ...
      # _elements is a single item
      get_filename_component(_result ${_elements} DIRECTORY)
      set (${_dir} ${_result} PARENT_SCOPE)
    endif()
  endfunction()
endif()
