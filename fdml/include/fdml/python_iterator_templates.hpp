...
inline bp::object pass_through(bp::object const& o) {
    return o;
}

// these template classes are used to allow more natural iteration in python

template <typename circulator> class Iterator_from_circulator {
  private:
    bool first = true;
    circulator m_first;
    circulator m_curr;
...
template <typename iterator> class Iterator_of_circulators {
...
template <typename iterator> void bind_iterator_of_circulators(const char* python_name) {
    bp::class_<iterator>(python_name, bp::no_init)
        .def("__iter__", &pass_through)
        .def("__next__", &iterator::next, bp::return_value_policy<bp::manage_new_object>());
}

template <typename iterator> void bind_iterator(const char* python_name) {
    bp::class_<iterator>(python_name, bp::no_init)
        .def("__iter__", &pass_through)
...
template <typename iterator> class CopyIterator {
  private:
    iterator m_curr;
    iterator m_end;
...
template <typename circulator> class Copy_iterator_from_circulator {
  private:
    bool first = true;
    circulator m_first;
    circulator m_curr;
...
template <typename iterator> void bind_copy_iterator(const char* python_name) {
    bp::class_<iterator>(python_name, bp::no_init).def("__iter__", &pass_through).def("__next__", &iterator::next);
}
#endif
